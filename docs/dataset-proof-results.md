...
4. **Truncated Thai columns.** Resolved on 23 September 2026 (ADR-0024): the product owner confirmed `สถ_1` as the centre name and `รอง` as its capacity. `ละต` and `ลอง` duplicate the geometry and are used only as a cross-check. `สถา` is GDAL's other truncation of the same `สถ...` pair and is not shown separately.
