...
- **AI:** every model call (routing and drafting) goes through `api/ai_gateway.py`. So the allowance, `llm_usage`, Section 10.6 messages and Langfuse export all apply. The model proposes a route; GRP checks access and inputs and writes the refusal text itself. A call starts only if its full estimated reservation fits the remaining allowance; actual usage is still fully counted under AI-09.
...
- **Records:** `planning_sig_evidence`, `planning_sig_area_rejected`, `sig_receipt_published` and `sig_receipt_blocked` in the security log. Chat text is not stored. *(Amended by ADR-0029, 25 September 2026: the conversation and the SIG evidence behind it are now stored per person and Hub, and the person can delete them.)*
