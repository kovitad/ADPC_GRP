...
**Problem.** `grp` is the name of a Python standard-library module: the Unix group database. On CPython builds where it is compiled in, a built-in module beats every entry on `sys.path`, so a directory named `grp/` can never be imported, whatever the path order. Confirmed on 21 September for two interpreters — uv-managed CPython 3.12 and Debian's `/usr/bin/python3.12` — where `'grp' in sys.builtin_module_names` is `True`. `from grpcli.admin import ...` then raises:
